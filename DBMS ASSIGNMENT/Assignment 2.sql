...
INNER JOIN tb_section ON tb_teaches.sec_id = tb_section.sec_id
INNER JOIN tb_course ON tb_section.course_id = tb_course.course_id
...
INNER JOIN tb_section ON tb_takes.sec_id = tb_section.sec_id
INNER JOIN tb_course ON tb_section.course_id = tb_course.course_id
...
-- using relational algebra
σ MAX(salary) (tb_instructor) GROUPBY dept_name
...
SELECT tb_instructor.dept_name, MAX(salary) as maximum_per_dept_salary
FROM tb_instructor
GROUP BY dept_name
)
...
FROM tb_student
INNER JOIN tb_advisor ON tb_student.student_id = tb_advisor.student_id
WHERE  tb_advisor.instructor_id IS NULL;
